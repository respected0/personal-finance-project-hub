...
- **Yasaklar**: Doğrudan veritabanına bağlanamaz. Kanonik finansal hesaplama yapamaz. Web kodunu kör biçimde kopyalayamaz. OpenAPI sözleşmesinin bağımsız kopyasını tutamaz. Kesin finans kaydını cihaz üzerinde tek başına oluşturamaz (offline veriler yalnızca açıkça taslak/DRAFT olabilir; kesin kaydolma backend commit yanıtından sonra gerçekleşir).
...
- **Finansal Doğruluk**: Parasal hesaplamalarda asla IEEE 754 `number` kullanılmaz; backend domain motoru `decimal.js` ile kesin hesaplama yapar.
- **Yalın Mimari Karar Yönetimi**: Mimari sınırlar ve entegrasyon protokolleri lightweight Architecture Decision Records (ADR) üzerinden şeffaf ve kanıtlanabilir biçimde yönetilir.
